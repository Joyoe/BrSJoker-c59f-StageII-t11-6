...
check_data() {
  ...
  if grep ' /data ' /proc/mounts | grep -vq 'tmpfs'; then
    ...
    $DATA_DE && [ -d /data/adb/joker ] || mkdir /data/adb/joker || DATA_DE=false
  fi
  NVBASE=/data
...
find_magisk_apk() {
  local DBAPK
  [ -z $APK ] && APK=/data/adb/joker.apk
  [ -f $APK ] || APK=/data/joker/joker.apk
  [ -f $APK ] || APK=/data/app/com.brightsight.joker*/*.apk
  [ -f $APK ] || APK=/data/app/*/com.brightsight.joker*/*.apk
  if [ ! -f $APK ]; then
    DBAPK=$(magisk --sqlite "SELECT value FROM strings WHERE key='requester'" 2>/dev/null | cut -d= -f2)
    [ -z $DBAPK ] && DBAPK=$(strings /data/adb/joker.db | grep -oE 'requester..*' | cut -c10-)
...
run_migrations() {
  ...
  local BACKUP=/data/adb/joker/stock_boot*.gz
  if [ -f $BACKUP ]; then
    cp $BACKUP /data
    rm -f $BACKUP
  fi
  ...
  for gz in /data/stock_boot*.gz; do
    ...
    [ -z $LOCSHA1 ] && break
    mkdir /data/joker_backup_${LOCSHA1} 2>/dev/null
    mv $gz /data/joker_backup_${LOCSHA1}/boot.img.gz
  done

  # Stock backups
  LOCSHA1=$SHA1
  for name in boot dtb dtbo dtbs; do
    BACKUP=/data/adb/joker/stock_${name}.img
    [ -f $BACKUP ] || continue
    if [ $name = 'boot' ]; then
      LOCSHA1=`$MAGISKBIN/jokerboot sha1 $BACKUP`
      mkdir /data/joker_backup_${LOCSHA1} 2>/dev/null
    fi
    TARGET=/data/joker_backup_${LOCSHA1}/${name}.img
    cp $BACKUP $TARGET
    rm -f $BACKUP
    gzip -9f $TARGET
  done
...
copy_sepolicy_rules() {
  ...
  rm -rf /data/unencrypted/magisk /cache/magisk /metadata/joker /persist/magisk /mnt/vendor/persist/magisk

  # Find current active RULESDIR
  local RULESDIR
  local active_dir=$(magisk --path)/.joker/mirror/sepolicy.rules
  if [ -L $active_dir ]; then
    RULESDIR=$(readlink $active_dir)
    [ "${RULESDIR:0:1}" != "/" ] && RULESDIR="$(magisk --path)/.joker/mirror/$RULESDIR"
