...
if mount | grep -q rootfs; then
  ...
  # Legacy SAR
  mount_sbin
  if ! grep -q '/sbin/.joker/mirror/system_root' /proc/mounts; then
    mkdir -p /sbin/.joker/mirror/system_root
    block=$(mount | grep ' / ' | awk '{ print $1 }')
    [ $block = "/dev/root" ] && block=/dev/block/dm-0
    mount -o ro $block /sbin/.joker/mirror/system_root
  fi
  for file in /sbin/.joker/mirror/system_root/sbin/*; do
    [ ! -e $file ] && break
    if [ -L $file ]; then
      cp -af $file /sbin
    else
