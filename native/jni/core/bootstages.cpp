...
static bool magisk_env() {
    ...
    const char *alt_bin[] = { "/cache/data_adb/joker", "/data/joker", buf };
    ...
    unlink("/data/adb/joker.img");
    unlink("/data/adb/joker_merge.img");
    unlink("/data/joker.img");
    unlink("/data/joker_merge.img");
    unlink("/data/joker_debug.log");
...
void boot_complete(int client) {
    ...
    if (!check_manager()) {
        if (access(MANAGERAPK, F_OK) == 0) {
            ...
            rename(MANAGERAPK, "/data/joker.apk");
            install_apk("/data/joker.apk");
        } else {
            // Install stub
            auto init = MAGISKTMP + "/jokerinit";
            exec_command_sync(init.data(), "-x", "manager", "/data/joker.apk");
            install_apk("/data/joker.apk");
        }
    }
    unlink(MANAGERAPK);
